import { Direction } from '@/lib/definitions';
import { useEffect, useState } from 'react';

type FaucetProps = {
  i?: number;
  j?: number;
  sizeX?: number;
  sizeY?: number;
  side?: Direction;
  sizeRatio?: number;
  animate?: boolean;
  status?: string;
  onClickHandler?: () => void;
};

const valvePath = `M332.04 217.39c10.77 6.32 20.28 13.5 28.51 21.3a141.02 141.02 0 0 1 19.19 22.38h44.38V235.4c0-5.59 2.28-10.68 5.96-14.35 3.67-3.68 8.76-5.96 14.35-5.96h47.25c5.59 0 10.69 2.28 14.36 5.96 3.67 3.67 5.96 8.76 5.96 14.35v215.04c0 5.59-2.29 10.69-5.96 14.36a20.257 20.257 0 0 1-14.36 5.96h-47.25c-5.59 0-10.68-2.28-14.35-5.96a20.257 20.257 0 0 1-5.96-14.36v-25.66h-45.83c-12.54 18.36-29.41 34.05-50.06 45.12-20.35 10.91-44.29 17.3-71.28 17.3-26.98 0-51.01-6.41-71.48-17.32-20.7-11.06-37.66-26.75-50.27-45.1H87.89v25.66c0 5.59-2.29 10.69-5.96 14.36l-.68.62c-3.62 3.31-8.43 5.34-13.68 5.34H20.32c-5.59 0-10.69-2.28-14.36-5.96A20.299 20.299 0 0 1 0 450.44V235.4c0-5.59 2.29-10.68 5.96-14.35 3.67-3.68 8.77-5.96 14.36-5.96h47.25c5.59 0 10.68 2.28 14.36 5.96 3.67 3.67 5.96 8.76 5.96 14.35v25.67h47.18c5.97-8.33 13.05-16.24 21.26-23.58 9.05-8.1 19.46-15.45 31.26-21.84 5.19-2.8 9.26-5.85 11.93-8.86 1.86-2.11 2.93-3.97 3-5.35l-.04-.17c0-1.69-1.18-3.48-2.98-4.94-2.32-1.88-5.47-3.09-8.75-3.16l-5.77.03c-9.15 0-17.49-3.76-23.54-9.81-6.05-6.01-9.81-14.36-9.81-23.54 0-9.17 3.76-17.53 9.79-23.56l.58-.54c6-5.72 14.12-9.25 22.98-9.25h16.13l30.39-49.34v-3.95h-90.31c-10.07 0-19.24-4.12-25.86-10.74-6.61-6.62-10.74-15.79-10.74-25.86 0-10.08 4.13-19.25 10.74-25.87C121.95 4.13 131.12 0 141.19 0h233.83c10.07 0 19.24 4.13 25.86 10.74 6.62 6.62 10.74 15.79 10.74 25.86 0 10.08-4.12 19.25-10.74 25.87-6.62 6.62-15.79 10.74-25.86 10.74h-85.58v6.99l25.52 46.3h13.97c9.14 0 17.49 3.76 23.53 9.81 6.05 6.01 9.81 14.37 9.81 23.54 0 9.16-3.72 17.39-9.72 23.39l-.58.54c-5.93 5.65-14.01 9.2-22.86 9.39l-3.33.2c-5.32.47-8.74 1.69-10.62 3.32-1.01.88-1.49 2.03-1.53 3.27l.05.79c.73 8.09 5.96 10.53 11.37 13.05l.69.36c2.05.96 4.13 1.96 6.3 3.23zm81.6 61.87v127.33h19.57c5.03 0 9.1 4.07 9.1 9.09v34.76c0 .57.25 1.12.63 1.5.38.38.92.63 1.49.63h47.25c.57 0 1.12-.25 1.5-.63.38-.38.63-.93.63-1.5V235.4c0-.57-.25-1.11-.63-1.49a2.16 2.16 0 0 0-1.5-.63h-47.25c-.57 0-1.11.25-1.49.63-.38.38-.63.92-.63 1.49v34.77c0 5.02-4.07 9.09-9.1 9.09h-19.57zm-15.73 127.33V279.26h-23c-3-.03-5.92-1.52-7.64-4.22-5.23-8.2-11.63-16.01-19.22-23.2-7.29-6.92-15.69-13.26-25.18-18.82-1.43-.84-3.1-1.65-4.79-2.44l-.71-.29c-10.26-4.79-20.16-9.41-21.8-27.98l-.06-1.7c-.14-6.63 2.24-12.8 7.72-17.57 4.69-4.08 11.59-6.96 20.99-7.79l4.53-.2c4.07-.08 7.8-1.71 10.58-4.29l.36-.39c2.71-2.7 4.39-6.41 4.39-10.52 0-4.15-1.72-7.95-4.46-10.7a15.02 15.02 0 0 0-10.69-4.46H184.98c-3.99 0-7.64 1.56-10.32 4.07l-.37.39c-2.75 2.75-4.47 6.55-4.47 10.7 0 4.16 1.72 7.95 4.47 10.7 2.72 2.75 6.51 4.46 10.69 4.46.92 0 5.33-.15 5.88.04 7.48.09 14.72 2.87 20.08 7.22 5.89 4.76 9.74 11.51 9.74 19l-.04.28c-.08 5.91-2.75 11.83-7.54 17.24-4.1 4.61-9.87 9.04-16.91 12.85-10.57 5.72-19.83 12.24-27.78 19.35-8.15 7.28-15.05 15.26-20.71 23.71a9.078 9.078 0 0 1-7.89 4.56h-25.72v127.33h25.96c2.99.03 5.92 1.51 7.64 4.22 11.24 17.64 26.9 32.72 46.31 43.08 17.85 9.53 39 15.12 62.95 15.12 23.94 0 45-5.58 62.76-15.1 19.12-10.24 34.56-25.12 45.71-42.56a9.096 9.096 0 0 1 8.01-4.76h24.48z`;

export default function Faucet({
  i,
  j,
  sizeX = 3,
  sizeY = 4,
  side = 'left',
  sizeRatio = 0.6,
  animate = true,
  status = 'closed',
  onClickHandler,
}: FaucetProps) {
  const [isOpen, setIsOpen] = useState(status === 'open');
  const [isBlocked, setIsBlocked] = useState(status === 'blocked');

  useEffect(() => {
    // debug
  }, []);

  useEffect(() => {
    setIsOpen(status === 'open');
    setIsBlocked(status === 'blocked');
  }, [status]);

  // fallback full-area faucet when coordinates are not provided
  if (typeof i !== 'number' || typeof j !== 'number') {
    return (
      <div
        style={{
          position: 'absolute',
          inset: 0,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          transform: 'rotate(90deg)',
        }}
      >
        <svg viewBox="0 0 512 487.2" width="64" height="64">
          <path d={valvePath} fill="#cfd8dc" stroke="#455a64" strokeWidth={6} />
        </svg>
      </div>
    );
  }

  // Compute percent-based placement and size relative to the grid
  const cellW = 100 / sizeX;
  const cellH = 100 / sizeY;
  const pieceLeft = i * cellW;
  const pieceTop = j * cellH;
  const faucetW = cellW * sizeRatio;
  const faucetH = cellH * sizeRatio;
  // center by default
  let left = pieceLeft + (cellW - faucetW) / 2;
  let top = pieceTop + (cellH - faucetH) / 2;
  // position slightly outside the cell depending on side
  if (side === 'left') {
    // place entirely to the left so its right edge touches the cell left edge
    left = pieceLeft - faucetW;
  } else if (side === 'right') {
    // place entirely to the right so its left edge touches the cell right edge
    left = pieceLeft + cellW;
  } else if (side === 'up') {
    // place entirely above so its bottom edge touches the cell top edge
    top = pieceTop - faucetH;
  } else if (side === 'down') {
    // place entirely below so its top edge touches the cell bottom edge
    top = pieceTop + cellH;
  }

  const containerStyle: React.CSSProperties = {
    position: 'absolute',
    left: `${left}%`,
    top: `${top}%`,
    width: `${faucetW}%`,
    height: `${faucetH}%`,
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    cursor: 'pointer',
    zIndex: 20,
    overflow: 'visible',
    pointerEvents: 'auto',
  };

  // viewBox dimensions from the provided SVG
  const vbW = 100;
  const vbH = 100;

  return (
    <div style={containerStyle} onClick={onClickHandler} title={`Faucet (${i},${j}) ${side}`}>
      <style>{`@keyframes faucet-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
      <svg viewBox={`0 0 ${vbW} ${vbH}`} width="100%" height="100%" style={{ overflow: 'visible' }}>
        {(() => {
          const cx = vbW / 2;
          const cy = vbH / 2;
          let outer = 0;
          if (side === 'right') outer = 180;
          else if (side === 'up') outer = 90;
          else if (side === 'down') outer = -90;
          const fill = isOpen ? '#29b6f6' : '#ffffff';
          const base = isBlocked ? '#eceff1' : '#eceff1';
          const stroke = '#37474f';
          return (
            <g transform={`rotate(${outer} ${cx} ${cy})`}>
              {/* top-down faucet: short spout (rectangle) and small rotating flange at the extremity */}
              <rect
                x={72}
                y={44}
                width={24}
                height={12}
                rx={2}
                fill={'#29b6f6'}
                stroke={stroke}
                strokeWidth={1.5}
              />
              <rect
                x={92}
                y={40}
                width={11}
                height={20}
                rx={4}
                fill={isOpen ? '#29b6f6' : base}
                stroke={stroke}
                style={{ transition: 'fill 200ms ease' }}
              />

              {/* small circular flange with an X that spins when open; placed at the spout extremity */}
              <g
                className="faucet-spinner"
                style={{
                  transformBox: 'fill-box',
                  transformOrigin: '6px 4px',
                  animation: isOpen ? 'faucet-spin 600ms linear infinite' : undefined,
                }}
              >
                <circle
                  cx={94}
                  cy={50}
                  r={4}
                  fill={isOpen ? '#81d4fa' : isBlocked ? '#eceff1' : '#ffffff'}
                  stroke={stroke}
                  strokeWidth={1}
                />
                <line
                  x1={90}
                  y1={48}
                  x2={94}
                  y2={52}
                  stroke={stroke}
                  strokeWidth={1.8}
                  strokeLinecap="round"
                />
                <line
                  x1={90}
                  y1={52}
                  x2={94}
                  y2={48}
                  stroke={stroke}
                  strokeWidth={1.8}
                  strokeLinecap="round"
                />
              </g>
            </g>
          );
        })()}
      </svg>
    </div>
  );
}
